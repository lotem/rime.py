...
class DB:
    ...
    def update_setting(cls, key, value):
        if cls.read_only:
            return False
        path = cls.__conn.execute(QUERY_SETTING_PATH_SQL, {'path': key}).fetchone()
        if path:
            path_id = path[0]
        else:
            c = cls.__conn.execute(ADD_SETTING_PATH_SQL, {'path': key})
            path_id = c.lastrowid
        if cls.read_setting(key) is None:
            cls.__conn.execute(ADD_SETTING_VALUE_SQL, {'path_id': path_id, 'value': value})
        else:
            cls.__conn.execute(UPDATE_SETTING_VALUE_SQL, {'path_id': path_id, 'value': value})
        cls.flush(True)
        return True
